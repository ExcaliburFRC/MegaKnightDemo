...
import static frc.robot.subsystems.intake.IntakeConstants.POSITION_TOLERANCE_RAD;
...
public class ArmSubsystem extends SubsystemBase implements Logged {
    ...
    public ArmSubsystem(Trigger isLeftRiffScore) {
        ...
        armMotor.setPositionConversionFactor(((1 / 15) * Math.PI * 2) / 1 * 1); // The numbers are just a placeholder
        ...
        limitsConfigs.SupplyCurrentLimit = 0; // The number is just a placeholder
        ...
        armMechanism = new Arm(
                ...
                angleSupplier,
                VELOCITY_LIMIT,
                new Gains(0, 0, 0, 0, 0, 0, 0), // The number is just a placeholder
                new Mass(
                ...
                () -> {
                    ...
                    if (heightDiff < 0) {
                        return limitHelper.getSetpoint(
                                angleSupplier.getAsDouble(), Math.PI / 2) + 1 - Math.PI / 2; // The number is just a placeholder
                    }
